...
public class WikipediaHomePage extends WikipediaHomePageBase {
    ...
    private static final Logger LOGGER = (Logger) LogManager.getLogger(WikipediaHomePage.class);
    ...
    public boolean hasExternalLinks() {
        try {
            ...
            for (String selector : externalLinkSelectors) {
                ...
                if (!links.isEmpty()) {
                    LOGGER.info("Found " + links.size() + " links with selector: " + selector);
                    for (int i = 0; i < Math.min(links.size(), 3); i++) {
                        String href = links.get(i).getAttribute("href");
                        LOGGER.info("  Link " + (i+1) + ": " + href);
                    }
                    totalLinksFound += links.size();
                }
            }

            LOGGER.info("Total external links found: " + totalLinksFound);
            return totalLinksFound > 0;

        } catch (Exception e) {
            LOGGER.error("Error checking external links: " + e.getMessage());
            e.printStackTrace();
            return false;
        }
    }
    ...
    public boolean hasLanguageLinks() {
        try {
            ...
            LOGGER.error("Error checking language links: " + e.getMessage());
            return false;
        }
    }
    ...
    public int getLanguageLinkCount() {
        try {
            ...
            LOGGER.error("Error counting language links: " + e.getMessage());
            return 0;
        }
    }
    ...
    public boolean hasHeadings() {
        try {
            ...
            LOGGER.error("Error checking headings: " + e.getMessage());
            return false;
        }
    }
    ...
    public boolean hasAnyNavigationElements() {
        try {
            ...
            LOGGER.error("Error checking navigation: " + e.getMessage());
            return false;
        }
    }
    ...
    public boolean hasAnyImages() {
        try {
            ...
            LOGGER.error("Error checking images: " + e.getMessage());
            return false;
        }
    }
    ...
    public boolean hasImageContainers() {
        try {
            ...
            LOGGER.error("Error checking image containers: " + e.getMessage());
            return false;
        }
    }
    ...
    public boolean isFooterPresent() {
        try {
            ...
            LOGGER.error("Error checking footer: " + e.getMessage());
            return false;
        }
    }
    ...
    public boolean hasViewHistoryLink() {
        try {
            ...
            LOGGER.error("Error checking view history link: " + e.getMessage());
            return false;
        }
    }
    ...
    public String getSearchInputValue() {
        try {
            ...
            LOGGER.error("Error getting search input value: " + e.getMessage());
            return "";
        }
    }
